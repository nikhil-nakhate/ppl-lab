...
%token <Support.Error.info> ISMEMBER
%token <Support.Error.info> INTERSECTION
%token <Support.Error.info> INSERT
%token <Support.Error.info> SET
%token <Support.Error.info> NIL
%token <Support.Error.info> SETTYPE
...
Term :
  ...
  | SET LPAREN Term COMMA Term RPAREN
      { fun ctx -> TmSet($1, $3 ctx, $5 ctx) }
  | ISMEMBER LPAREN Term COMMA Term RPAREN
      { fun ctx -> TmIsMember($1, $3 ctx, $5 ctx) }
  | INTERSECTION LPAREN Term COMMA Term RPAREN
      { fun ctx -> TmIntersection($1, $3 ctx, $5 ctx) }
  | INSERT LPAREN Term COMMA Term RPAREN
      { fun ctx -> TmInsert($1, $3 ctx, $5 ctx) }
  | CASE Term OF Cases
      { fun ctx ->
...
Field :
    LCID EQ Term
      { fun ctx i -> ($1.v, $3 ctx) }                   (*record field and the next one is tuple field *)
  | Term
      { fun ctx i -> (string_of_int i, $1 ctx) }
