...
let rec iseq v1 v2 = match (v1,v2) with 
    (TmZero(_),TmZero(_)) -> true
  | (TmZero(_),TmSucc(_,_)) -> false 
  | (TmSucc(_,_),TmZero(_)) -> false
  | (TmSucc(_,t1),TmSucc(_,t2)) -> iseq t1 t2
  | _ -> false
...
let rec isset ctx t = match t with
    TmSet(_,t1,nil) -> true
  | _ -> false
...
let rec eval1 ctx t = match t with
  ...
  | TmIf(fi,t1,t2,t3) ->
      ...
      TmIf(fi, t1', t2, t3)
  | TmSet(fi,v1,t2) when isnumericval ctx v1-> let t2' = eval1 ctx t2 in TmSet(fi,v1,t2')
  | TmSet(fi,t1,t2) -> let t1' = eval1 ctx t1 in TmSet(fi,t1',t2) 
  | TmIsMember(_,v1,TmNil(_)) when isval ctx v1 -> TmFalse(dummyinfo)
  | TmIsMember(_,v1,TmSet(_,v2,s2)) when iseq v1 v2 -> TmTrue(dummyinfo)
  | TmIsMember(_,v1,TmSet(_,v2,s2)) when isval ctx v1 && isval ctx v2 -> TmIsMember(dummyinfo,v1,s2)
  | TmIntersection(_,TmNil(_),t2) -> TmNil(dummyinfo)  
  | TmIntersection(_,t1,TmNil(_)) -> TmNil(dummyinfo)
  | TmIntersection(fi,TmSet(_,v1,s1),s2) -> TmIf(fi,TmIsMember(fi,v1,s2),TmSet(fi,v1,TmIntersection(fi,s1,s2)),TmIntersection(fi,s1,s2))
  | TmInsert(_,v1,TmNil(_)) -> TmSet(dummyinfo,v1,TmNil(dummyinfo))
  | TmInsert(_,v1,s1) -> TmIf(dummyinfo,TmIsMember(dummyinfo,v1,s1),s1,TmSet(dummyinfo,v1,s1))
  | TmTag(fi,l,t1,tyT) ->
      let t1' = eval1 ctx t1 in
      TmTag(fi, l, t1',tyT)
...
let rec typeof ctx t =
  ...
  | TmFalse(fi) -> 
      TyBool
  | TmNil(fi) -> TySettype
  | TmSet(fi,t1,t2) -> TySettype
  | TmIsMember(fi,t1,t2) -> TyBool
  | TmIntersection(fi,t1,t2) -> TySettype
  | TmInsert(fi,t1,t2) ->  TySettype
  | TmIf(fi,t1,t2,t3) ->
