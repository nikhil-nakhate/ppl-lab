...
type term =
  ...
  | TmFalse of info
  | TmNil of info
  | TmSet of info * term * term
  | TmIsMember of info * term * term
  | TmInsert of info * term * term
  | TmIntersection of info * term * term
...
let tmmap onvar ontype c t = 
  ...
  | TmSet(fi,t1,t2) -> TmSet(fi, walk c t1, walk c t2)
  | TmIsMember(fi,t1,t2) -> TmIsMember(fi, walk c t1, walk c t2)
  | TmInsert(fi,t1,t2) -> TmInsert(fi, walk c t1, walk c t2)
  | TmIntersection(fi,t1,t2) -> TmIntersection(fi, walk c t1, walk c t2)
...
let tmInfo t = match t with
  ...
  | TmNil(fi) -> fi
  | TmSet(fi,_,_) -> fi
  | TmIsMember(fi,_,_) -> fi
  | TmInsert(fi,_,_) -> fi
  | TmIntersection(fi,_,_) -> fi
  | TmIf(fi,_,_,_) -> fi
  | TmTag(fi,_,_,_) -> fi
  | TmCase(fi,_,_) -> fi
  | TmVar(fi,_,_) -> fi
