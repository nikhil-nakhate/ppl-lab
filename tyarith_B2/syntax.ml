...
and printty_AType outer tyT = match tyT with
    TyBool -> pr "Bool"
  | TyNat -> pr "Nat"
  | TyWeekend -> pr "weekend"
  | TyWeekday -> pr "weekday"
...
and printtm_AppTerm outer t = match t with
  ...
  | TmIsWeekday(_,t1) ->
       pr "isweekday "; printtm_ATerm false t1
  | TmNextWeekday(_,t1) ->
       pr "nextweekday "; printtm_ATerm false t1
  | t -> printtm_ATerm outer t

and printtm_ATerm outer t = match t with
    TmTrue(_) -> pr "true"
  | TmFalse(_) -> pr "false"
  | TmMonday(_) -> pr "monday"
  | TmTuesday(_) -> pr "tuesday"
  | TmWednesday(_) -> pr "wednesday"
  | TmThursday(_) -> pr "thursday"
  | TmFriday(_) -> pr "friday"
  | TmSaturday(_) -> pr "saturday"
  | TmSunday(_) -> pr "sunday"
  | TmZero(fi) ->
       pr "0"
  | TmSucc(_,t1) ->
